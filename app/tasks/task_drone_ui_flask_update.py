...
class DroneUIFlaskUpdateTask(TaskInterface):
    def __init__(self, drone=None, drone_id=None, local_file=None, version=None):
        ...
        self._procedure_name = "Drone-UI-Flask"
        self._local_image_file = local_file
        self.set_task_status_ready()

    def _go_internal(self) -> bool:
        self.set_task_status_in_progress()
        self.logger.info("DroneUIFlaskTask go")
        success_config = self.get_drone().cc.update_drone_ui_flask(self._local_image_file)
        return self._set_state_based_on_success(success_config)
