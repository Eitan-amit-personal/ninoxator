...
class ArdupilotFirmwareIngredient(Ingredient):
    ...
    def prepare_tasks(self) -> List[TaskInterface]:
        self.logger.debug(f"ArdupilotFirmwareIngredient prepare_tasks firmware_file={self.__full_file_path}")
        return self.task_with_drone(FlightControllerFirmwareUpdateTask, local_file=self.__full_file_path)
