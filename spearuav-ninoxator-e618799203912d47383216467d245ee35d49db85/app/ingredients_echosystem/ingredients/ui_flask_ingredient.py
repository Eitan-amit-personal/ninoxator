...
class UiFlaskIngredient(Ingredient):
    ...
    def prepare_tasks(self) -> List[TaskInterface]:
        self.logger.debug(f"UiFlaskIngredient prepare_tasks source file path={self.__source_file_path}")
        return self.task_with_drone(DroneUIFlaskUpdateTask, local_file=self.__source_file_path)
